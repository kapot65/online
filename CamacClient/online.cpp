...
{
    currSubFolder = tr("%1/%2_%3").arg(session).arg(group).arg(iteration);
    if(add_time)
...
void Online::savePoint(MachineHeader machineHeader, QVariantMap meta, QVector<Event> data)
{
    ...
    if(ext_meta.contains("HV1_value"))
        filename += tr("(HV1=%1)").arg(ext_meta["HV1_value"].toInt());
    if(ext_meta.contains("HV2_value"))
...
void HVMonitor::prepareVoltageFile()
{
    stopHvMonitorFlag = 0;
...
//    QVariantMap voltage;
//    voltage["timestamp"] = QString().number(QDateTime::currentDateTime().toMSecsSinceEpoch());
//    voltage["block"] = message["block"];
//    voltage["voltage"] = message["voltage"];

//#ifdef USE_QTJSON
//    QByteArray data = QJsonDocument::fromVariant(voltage).toJson();
//#else
//    QJson::Serializer serializer;
//    serializer.setIndentMode(QJson::IndentFull); // в настройки
//    QByteArray data = serializer.serialize(voltage);
//#endif
//    voltageFile->write(data);
//    voltageFile->flush();
}
